...
// pgDB wires the DB interface to a pgxpool.Pool. A separate connection
// (listenConn) holds the LISTEN session for the {schema}_queue_event channel.
type pgDB struct {
	pool       *pgxpool.Pool
	schema     string
	listenConn *pgx.Conn
...
// queryer is the subset of pgx methods we use; matches both *pgxpool.Pool
// and pgx.Tx so writes/reads can run inside WithWorkerLock's transaction.
